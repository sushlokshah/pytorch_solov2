...
def train(epoch_iters=1, total_epochs=36):
    ...
    training_transforms = build_process_pipeline(cfg.train_pipeline)
    val_transforms = build_process_pipeline(cfg.test_pipeline)
    # print("training_transforms:", training_transforms)
    # print("val_transforms:", val_transforms)
    # exit()
    # #build datashet
    train_data = CocoDataset(
        ann_file=cfg.dataset.train_info,
        pipeline=training_transforms,
        img_prefix=cfg.dataset.trainimg_prefix,
        data_root=cfg.dataset.data_root,
    )

    val_data = CocoDataset(
        ann_file=cfg.dataset.val_info,
        pipeline=val_transforms,
        img_prefix=cfg.dataset.valimg_prefix,
        data_root=cfg.dataset.data_root,
    )
    ...
    for i, data in enumerate(train_data_loader):
        ...
        print(len(data["gt_masks"]))
        break

    # exit()
    batchsize = cfg.imgs_per_gpu * cfg.num_gpus

    epoch_size = len(train_data) // batchsize
    step_index = 0
    # print("epoch_size:", epoch_size, len(train_data_loader))
    # print(batchsize)
    # exit()
    ...
    if epoch_iters < cfg.lr_config["step"][0]:
        ...
        epoch_iters >= cfg.lr_config["step"][0]
        and epoch_iters < cfg.lr_config["step"][1]
    ):
    ...
    try:
        for iter_nums in range(left_loops):
            ...
            if epoch_iters < cfg.lr_config["step"][0]:
                ...
                epoch_iters >= cfg.lr_config["step"][1] and epoch_iters <= total_epochs
            ):
                set_lr(optimizer, 0.0001)
                base_lr = 0.0001
                cur_lr = 0.0001
                ...
                raise NotImplementedError("train epoch is done!")
            print("running epoch:", iter_nums + base_loop)
            for j, data in enumerate(train_data_loader):
                if (
                    cfg.lr_config["warmup"] is not None
                    and base_nums < cfg.lr_config["warmup_iters"]
                ):
                ...
                if j % 50 == 0 and j != 0:
                    ...
                    print(
                        out_srt,
                        "loss: ",
                        format(loss_sum / 50.0, ".4f"),
                        " loss_ins:",
                        format(loss_ins / 50.0, ".4f"),
                        "loss_cate:",
                        format(loss_cate / 50.0, ".4f"),
                        "lr:",
                        format(cur_lr, ".5f"),
                    )
                    loss_sum = 0.0
...
if __name__ == "__main__":
    train(
        epoch_iters=cfg.epoch_iters_start, total_epochs=cfg.total_epoch
    )  # 设置本次训练的起始epoch
